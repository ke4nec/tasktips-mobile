...
- 首版 §6 已确认并实现：本地备份（SAF 导出/恢复 zip，仅 content，进包校验+非法路径拒绝+失败回滚，state/凭据不进包，恢复后按本机改动同步，见 lib/infra/backup.dart、test/backup_test.dart）；切换项目弹层二选一（并入目标项目走既有预览合并 / 放弃本地先快照到 recovery 再清空重拉，见 resetLocalAdoptRemote）。发布基线：minSdk 23（Android 6.0，workmanager 锁定）。
